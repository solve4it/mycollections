...
- **The two failure screens, and why they are two** (`apps/web/src/components/ErrorScreen.tsx`). `RouteError` is the `defaultErrorComponent`: a route's boundary replaces the *screen*, so the shell is still standing and it can offer a link back to /collections. `AppErrorScreen` is the root route's own `errorComponent` and the `ErrorBoundary`'s fallback: the root's boundary wraps the root component, so a throw in `Root` or `Shell` renders with no nav, no `<main>` landmark and nothing left maintaining `document.title` — it brings all three, and offers only Reload, since routing is not known to work from there. Neither takes the boundary's `reset`: it only clears boundary state, so a deterministic render bug throws again and reports twice, and `router.invalidate()` rebuilds every committed match, resetting the boundary once per store update on the way through.
- **The error screens announce themselves by taking focus, not by being a live region.** They carry `role="alert"` — that is how every failure in this app is written, and the danger treatment hangs off the role — but they are the one message that cannot follow the rule in "Live regions the announcer cannot carry" below, because the subtree a persistent region would live in is the one being replaced. A crash also destroys whatever was focused: focus falls to `<body>` and the tab order restarts at the top of the document (WCAG 2.4.3), and where the crash was a re-render rather than a navigation the shell's own focus recovery never runs. So the surface focuses itself, guarded on `document.activeElement` being `<body>` or nothing — the same guard `Shell.tsx` uses, which is what keeps the two from fighting and stops a double announcement where focus survived.
...
- **The one exception is a screen that replaces itself.** The two failure screens (#319) cannot be
  persistent-and-empty, because the subtree they would be persistent in is the one being replaced.
  They announce by taking focus instead — see "The error screens announce themselves by taking
  focus" above. Reach for this only when there is genuinely no surviving host; a conditional
  `aria-live` beside working UI is still the bug.
