...
- **Accessibility**: UI changes must keep `pnpm test:e2e` green — the Playwright + axe sweep over every route, at two viewports and both color schemes. It is intentionally outside `pnpm check` (it needs a browser); run it separately. A new route or a new interactive state gets a scan in `apps/web/e2e/a11y.spec.ts`. A new route also declares `staticData: { titleKey }` so it is named and announced — plus `dynamicTitle: true`, and a `usePageTitle` call in the screen, when the page's real name only arrives with its data — and no new live region is mounted with its text already inside: hidden messages go through the shell's announcer, a message that must be visible or carries a control (the undo toast, the import's result, the emptied trash) gets a class-named region of its own that is rendered unconditionally and filled afterwards, a notice that can never change is not a live region at all — all of which is about `aria-live`; a `role="alert"` is announced when it is inserted, so a failure surface renders with its message already inside it — see "Page titles and route announcements" and "Live regions the announcer cannot carry" in DEVELOPMENT.md. One-time setup: `pnpm --filter @mycollections/web exec playwright install chromium`.
