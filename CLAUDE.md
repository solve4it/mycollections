...
- **Accessibility**: UI changes must keep `pnpm test:e2e` green — the Playwright + axe sweep over every route, at two viewports and both color schemes. It is intentionally outside `pnpm check` (it needs a browser); run it separately. A new route or a new interactive state gets a scan in `apps/web/e2e/a11y.spec.ts`. One-time setup: `pnpm --filter @mycollections/web exec playwright install chromium`.
