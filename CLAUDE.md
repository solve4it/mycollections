...
- **Accessibility**: UI changes must keep `pnpm test:e2e` green — the Playwright + axe sweep over every route, at two viewports and both color schemes. It is intentionally outside `pnpm check` (it needs a browser); run it separately. A new route or a new interactive state gets a scan in `apps/web/e2e/a11y.spec.ts`. A new route also declares `staticData: { titleKey }` so it is named and announced, and no new live region is mounted with its text already inside: hidden messages go through the shell's announcer, and a message that must be visible or carries a control (the undo toast) gets a region of its own that is rendered unconditionally and filled afterwards — see "Page titles and route announcements" and "Live regions the announcer cannot carry" in DEVELOPMENT.md. One-time setup: `pnpm --filter @mycollections/web exec playwright install chromium`.
