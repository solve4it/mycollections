...
- **Images.** Put images in `docs/assets/` and reference them relatively — `![Shelf view](./assets/shelf.png)` — which is the form GitHub resolves against the file itself. The docs site copies `docs/assets/` in alongside the generated Markdown and hands every referenced image to Astro's image pipeline, so it reaches the built site optimized, re-encoded and content-hashed: `./assets/shelf.png` is published as `/mycollections/_astro/shelf.<hash>.webp`. Write the relative reference and let the build do that — never hardcode a URL under `_astro/`, and don't count on the original filename or format surviving. An image the pipeline cannot find fails the build instead of shipping a broken page.
